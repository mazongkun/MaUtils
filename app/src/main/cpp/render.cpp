...
int render::renderToScreen(unsigned char* buffer, int width, int height, bool isFrontCamera) {
    LOGD("%s renderToScreen: %dx%d, %d, [%d, %d, %d]", __func__, width, height, isFrontCamera, m2DProgram, mTexture, mFramebuffer);
    ...
    if (this->frontCamera != isFrontCamera) {
        this->frontCamera = isFrontCamera;
        rotateTexture(mScreenTextureBuffer, 90, isFrontCamera, false);
    }
...
int render::renderTextureToScreen(int texture, int width, int height, bool isFrontCamera) {
...
render::render() : m2DProgram(-1), mFramebuffer(-1), mTexture(-1),
